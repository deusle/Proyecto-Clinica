...
# --- NUEVAS FUNCIONES PARA KPIs DE REPORTES ---
...
# --- Funciones existentes para reportes ---
...
def obtener_distribucion_uso_seguro(start_date, end_date):
    ...
    SELECT 
        CASE 
            WHEN aseguradora IS NOT NULL AND aseguradora != 'Ninguno' THEN 'Con Seguro' 
            ELSE 'Sin Seguro' 
        END as uso_seguro,
        COUNT(*) as count
    FROM citas
...
# --- El resto de funciones se mantienen como en la V2 original ---
def actualizar_consulta_clinica(cita_id, sintomas, diagnostico, tratamiento, notas):
    ...
    cursor.execute(query, (sintomas, diagnostico, tratamiento, notas, cita_id)); conn.commit(); conn.close()
def obtener_historial_clinico_paciente(paciente_id):
    conn = conectar_db(); cursor = conn.cursor()
    query = "SELECT c.fecha_hora, m.nombre_completo, m.especialidad, c.sintomas, c.diagnostico, c.tratamiento FROM citas c JOIN medicos m ON c.medico_id = m.id WHERE c.paciente_id = ? AND c.estado = 'Pagada' ORDER BY c.fecha_hora DESC"
...
def obtener_historial_pagos():
    conn = conectar_db(); cursor = conn.cursor()
    query = "SELECT c.id, p.nombre || ' ' || p.apellidos, m.nombre_completo, c.fecha_hora, c.monto_pagado, c.metodo_pago, c.aseguradora, c.sintomas, c.diagnostico, c.tratamiento, c.notas FROM citas c JOIN pacientes p ON c.paciente_id = p.id JOIN medicos m ON c.medico_id = m.id WHERE c.estado = 'Pagada' ORDER BY c.fecha_hora DESC"
...
def agregar_medico(nombre, especialidad, telefono, horarios):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("INSERT INTO medicos (nombre_completo, especialidad, telefono) VALUES (?, ?, ?)",(nombre, especialidad, telefono));medico_id=cursor.lastrowid
...
def actualizar_medico(medico_id,nombre,especialidad,telefono,horarios):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("UPDATE medicos SET nombre_completo = ?, especialidad = ?, telefono = ? WHERE id = ?",(nombre, especialidad, telefono, medico_id))
    cursor.execute("DELETE FROM horarios_medicos WHERE medico_id = ?",(medico_id,));
    for dia,horas in horarios.items():
        if horas: cursor.execute("INSERT INTO horarios_medicos (medico_id, dia_semana, hora_inicio, hora_fin) VALUES (?, ?, ?, ?)",(medico_id,dia,horas['inicio'],horas['fin']))
    conn.commit();conn.close()
def obtener_medicos(filtro=""):
    conn=conectar_db();cursor=conn.cursor();query="SELECT id, nombre_completo, especialidad, telefono FROM medicos";
...
def agregar_paciente(dni,nombre,apellidos,fecha_nac,genero,telefono,email,historial):
    conn = conectar_db(); cursor = conn.cursor(); cursor.execute("INSERT INTO pacientes (dni, nombre, apellidos, fecha_nac, genero, telefono, email, historial_basico) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (dni, nombre, apellidos, fecha_nac, genero, telefono, email, historial)); conn.commit(); conn.close()
def obtener_pacientes(filtro=""):
    conn=conectar_db();cursor=conn.cursor();query="SELECT * FROM pacientes";
...
def obtener_paciente_por_id(paciente_id):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("SELECT * FROM pacientes WHERE id = ?",(paciente_id,));paciente=cursor.fetchone();conn.close();return paciente
def actualizar_paciente(paciente_id,dni,nombre,apellidos,fecha_nac,genero,telefono,email,historial):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("UPDATE pacientes SET dni = ?, nombre = ?, apellidos = ?, fecha_nac = ?, genero = ?, telefono = ?, email = ?, historial_basico = ? WHERE id = ?",(dni,nombre,apellidos,fecha_nac,genero,telefono,email,historial,paciente_id));conn.commit();conn.close()
def obtener_medico_por_id(medico_id):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("SELECT * FROM medicos WHERE id = ?",(medico_id,));medico=cursor.fetchone();conn.close();return medico
def obtener_medicos_por_especialidad(especialidad):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("SELECT id, nombre_completo FROM medicos WHERE especialidad = ?",(especialidad,));medicos=cursor.fetchall();conn.close();return medicos
def obtener_cita_completa_por_id(cita_id):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("SELECT * FROM citas WHERE id = ?",(cita_id,));cita=cursor.fetchone();conn.close();return cita
def actualizar_estado_cita(cita_id,nuevo_estado):
    conn=conectar_db();cursor=conn.cursor();cursor.execute("UPDATE citas SET estado = ? WHERE id = ?",(nuevo_estado,cita_id));conn.commit();conn.close()